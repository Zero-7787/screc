...
impl HlsDownloader {
    ...
    /// 下载 HLS 流（直接下载为临时 TS，满 60 分钟或结束后，转码为 MP4）
        ...
        F: Fn(&str) -> String,
    {
        ...
        'record_session: loop {
                ...
                match round_result {
                    ...
                    Ok((RoundOutcome::PendingRetry, _)) => {
                        ...
                        if consecutive_pending_retries >= MAX_CONSECUTIVE_PENDING_RETRIES {
                            warn!("[{}] 连续 {} 轮分片下载失败，停止下载", self.username, MAX_CONSECUTIVE_PENDING_RETRIES);
                            break 'poll_loop Ok(());
                        }
            ...
            if has_downloaded_content {
                debug!("[{}] 正在将分段 {} 的 TS 文件转换为 MP4 格式...", self.username, part_index);
                // 【核心修改】直接拿临时 TS 文件进行转码
    ...
    /// 将临时的 TS 文件无损转换为 MP4 格式，修正时间戳
    async fn convert_ts_to_mp4(&self, ts_path: &Path, mp4_path: &Path) -> Result<()> {
        use std::process::Command;

        info!("[{}] 正在使用 FFmpeg 修正时间戳并无损转码为 MP4...", self.username);

        let mut std_cmd = Command::new("ffmpeg");
        std_cmd
            // 使用 genpts 和 igndts 修复可能因切片拼接导致的时间戳错误
            .arg("-fflags")
            .arg("+genpts+igndts")
            .arg("-i")
            .arg(ts_path)
