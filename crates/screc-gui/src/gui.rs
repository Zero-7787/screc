...
impl AppView {
    pub fn new(gui_state: SharedGuiState, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        // 拦截窗口关闭：录制中阻止关闭，请先用顶部开关停止录制
        let gs_close = gui_state.clone();
        window.on_window_should_close(&*cx, move |_window, _app| {
            if !gs_close.is_recording_active() {
                return true;
            }
            log::warn!("窗口关闭被阻止：当前正在录制中，请先停止录制后再关闭窗口");
            gs_close.push_log(
                crate::shared_state::LogLevel::Warn,
                "窗口关闭被阻止：请先停止录制后再关闭窗口".to_string(),
            );
            false
        });
    ...
    fn build_recording_page(&self) -> SettingPage {
        ...
        SettingPage::new("录制").default_open(true).groups(vec![
            SettingGroup::new().title("录制设置").items(vec![
                {
                    let s = state.clone();
                    SettingItem::new(
                        "输出目录",
    ...
    fn build_network_page(&self) -> SettingPage {
        ...
        SettingPage::new("网络").groups(vec![SettingGroup::new().title("网络与代理").items(vec![
            {
                let s = state.clone();
                SettingItem::new(
                    "代理地址",
                    SettingField::input(
                        {
                            let sc = s.clone();
                            move |_: &App| {
                                sc.with_app_config(|c| {
                                    SharedString::from(c.get_proxy().unwrap_or_default())
                                })
                            }
                        },
                        move |val: SharedString, _cx: &mut App| {
                    ...
                    SettingField::input(
                        {
                            ...
                            move |_: &App| {
                                sc.with_app_config(|c| {
                                    SharedString::from(c.get_proxy_username().unwrap_or_default())
                                })
                            }
                        },
                        move |val: SharedString, _cx: &mut App| {
                    ...
                    SettingField::input(
                        {
                            ...
                            move |_: &App| {
                                sc.with_app_config(|c| {
                                    SharedString::from(c.get_proxy_password().unwrap_or_default())
                                })
                            }
                        },
                        move |val: SharedString, _cx: &mut App| {
                ...
                SettingItem::new(
                    ...
                    SettingField::render({
                        ...
                        move |options, window, cx| {
                            let ua =
                                gui.with_app_config(|c| SharedString::from(c.get_user_agent()));
                            ...
                            let state = window
                                .use_keyed_state(SharedString::from(state_key), cx, |window, cx| {
                                    let input = cx.new(|cx| {
                            ...
                            let (input, _) = state;
                            Input::new(input).w_full().h(px(72.)).into_any_element()
                        }
                    }),
                )
                .description("新启动的录制任务生效，正在录制的模特需停止后重启")
    ...
    fn build_logging_page(&self) -> SettingPage {
        ...
        SettingPage::new("日志").groups(vec![SettingGroup::new().title("日志设置").items(vec![
            {
                let s = state.clone();
                SettingItem::new(
                    "调试日志",
    ...
    fn render_settings_page(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .gap_4()
            .child(
                Settings::new(format!("app-settings-v{}", self.gui_state.config_version()))
                    .pages(vec![recording_page, network_page, logging_page]),
            )
            .child(
                div()
                    .v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                div().flex_1().child(
                                    Input::new(&self.settings_new_config_path)
                                        .disabled(recording_active),
                                ),
                            )
                            .child({
                                ...
                                if !recording_active {
                                    btn =
                                        btn.on_click(cx.listener(move |this, _ev, _window, cx| {
                                            let path_str =
                                                new_path_input.read(cx).value().to_string();
                                            let path = std::path::PathBuf::from(path_str.trim());
                                            if path_str.trim().is_empty() {
                                                this.settings_config_switch_error =
                                                    Some("请输入配置文件路径".into());
                                                cx.notify();
                                                return;
                                            }
                    ...
                    .child(if let Some(ref err) = error_message {
                        div()
                            .text_xs()
